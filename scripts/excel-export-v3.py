"""
Excel 逐图导出 v3 — 直接用 Shape.CopyPicture + Chart 大尺寸导出
"""
import sys, os
sys.stdout.reconfigure(encoding='utf-8')
from pathlib import Path
from win32com.client import Dispatch, constants

EXCEL = r"C:\Users\26433\Desktop\南洋万邦实习材料\P100204013检规(1).xlsx"
OUT = Path(r"C:\Users\26433\Desktop\南洋万邦实习材料\knowledge-base\drawings")

SHEETS = {
    "P100206001": "P100206001",
    "P100206002": "P100204013",
    "P100206003集流管": "P100206003",
    "P100206004集流管": "P100206004",
    "P100206006翅片": "P100204006",
}

def main():
    print("Starting Excel...")
    excel = Dispatch("Excel.Application")
    excel.Visible = False
    excel.DisplayAlerts = False
    excel.ScreenUpdating = False

    try:
        wb = excel.Workbooks.Open(EXCEL, ReadOnly=True)
        total = 0

        for sheet_name, pid in SHEETS.items():
            ws = wb.Worksheets(sheet_name)
            out_dir = OUT / pid
            out_dir.mkdir(parents=True, exist_ok=True)

            for f in out_dir.glob("excel_export_*.png"):
                f.unlink()

            shapes = ws.Shapes
            print(f"\n{sheet_name} -> {pid} ({shapes.Count} shapes)")

            for i in range(1, shapes.Count + 1):
                shape = shapes.Item(i)
                if shape.Type not in (13, 6):
                    continue

                sh_name = shape.Name
                try:
                    # Method: CopyPicture as screen-render bitmap
                    shape.CopyPicture(1, 2)  # xlScreen=1, xlBitmap=2

                    # Create chart
                    chart_name = f"_X_{pid}_{i}"
                    try: excel.Charts(chart_name).Delete()
                    except: pass

                    chart = excel.Charts.Add()
                    chart.Name = chart_name

                    # Set chart area size to match shape (in points)
                    sh_w = shape.Width
                    sh_h = shape.Height

                    chart.ChartArea.Width = max(sh_w, 100)
                    chart.ChartArea.Height = max(sh_h, 100)

                    # Paste
                    chart.Paste()

                    out_path = str(out_dir / f"excel_export_{total+1:02d}.png")
                    chart.Export(out_path, "PNG")

                    chart.Delete()

                    sz = Path(out_path).stat().st_size
                    tag = "OK" if sz > 10240 else f"SMALL({sz}B)"
                    print(f"  {out_path.split(chr(92))[-1]} {tag} <-- {sh_name} ({int(sh_w)}x{int(sh_h)}pt)")
                    total += 1

                except Exception as e:
                    print(f"  ERR {sh_name}: {e}")
                    try: chart.Delete()
                    except: pass

        print(f"\nTOTAL: {total} images")

    finally:
        wb.Close(SaveChanges=False)
        excel.Quit()

if __name__ == "__main__":
    main()
